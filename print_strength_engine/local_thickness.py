...
def screen_contour_layers(layers,bounds,*,resolution_mm=.4,max_voxels=4_000_000,progress=None,cancelled=None):
    ...
    xx,yy=np.meshgrid(origin[0]+(np.arange(shape[2])+.5)*spacing,origin[1]+(np.arange(shape[1])+.5)*spacing)
    ...
    for index,row in enumerate(layers):
        ...
        parity=np.zeros(xx.shape,dtype=bool);walls=np.zeros(xx.shape,dtype=bool)
        for run in row.get('runs',[]):
            ...
            if len(points)>=4 and math.dist(points[0],points[-1])<=max(.05,width*.5):
                ...
                if polygon.area>width*width:
                    ...
                    parity^=shapely.contains_xy(polygon,xx,yy);closed_count+=1
                else:open_count+=1
            else:open_count+=1
            walls|=shapely.contains_xy(line.buffer(width/2,cap_style=2,join_style=2),xx,yy)
        plane=parity|walls
